...
TOSS_A_DICE = (rand() % 10000) / 10000.0; // random between 0 and 1
if (TOSS_A_DICE < pow(mypdf(y1_c2),0.25) /pow(mypdf(y1_now2),0.25) )
{
	y1_now2 = y1_c2;
...
// exchange
TOSS_A_DICE = (rand() % 10000) / 10000.0; // random between 0 and 1
   if (TOSS_A_DICE < ((mypdf(y1_now2)*(pow(mypdf(y1_now),0.25))) / (mypdf(y1_now)*( pow(mypdf(y1_now2),0.25)))))
       {
	   exchange1 = y1_now2;
