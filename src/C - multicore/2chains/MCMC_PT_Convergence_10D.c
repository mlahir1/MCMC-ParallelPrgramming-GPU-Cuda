...
double mypdf     (double x1,double x2,double x3,double x4,double x5,double x6,double x7,double x8,double x9,double x10);
double expectancy(double x1,double x2,double x3,double x4,double x5,double x6,double x7,double x8,double x9,double x10);
...
y8_c = y8_now + TUNED_VAR*sampleNormal();
y9_c = y9_now + TUNED_VAR*sampleNormal();
y10_c = y10_now + TUNED_VAR*sampleNormal();

TOSS_A_DICE = (rand() % 10000) / 10000.0; // random between 0 and 1
if (TOSS_A_DICE < mypdf(y1_c,y2_c,y3_c,y4_c,y5_c,y6_c,y7_c,y8_c,y9_c,y10_c) /mypdf(y1_now,y2_now,y3_now,y4_now,y5_now,y6_now,y7_now,y8_now,y9_now,y10_now) ) 
{
	y1_now = y1_c;
    y2_now = y2_c;
    y3_now = y3_c;
...
y7_c2 = y7_now2 + TUNED_VAR*sampleNormal();
y8_c2 = y8_now2 + TUNED_VAR*sampleNormal();
y9_c2 = y9_now2 + TUNED_VAR*sampleNormal();
y10_c2 = y10_now2 + TUNED_VAR*sampleNormal();

TOSS_A_DICE = (rand() % 10000) / 10000.0; // random between 0 and 1
if (TOSS_A_DICE < pow(mypdf(y1_c2,y2_c2,y3_c2,y4_c2,y5_c2,y6_c2,y7_c2,y8_c2,y9_c2,y10_c2),0.25) /pow(mypdf(y1_now2,y2_now2,y3_now2,y4_now2,y5_now2,y6_now2,y7_now2,y8_now2,y9_now2,y10_now2),0.25) )
{
	y1_now2 = y1_c2;
    y2_now2 = y2_c2;
    y3_now2 = y3_c2;
...
// exchange
TOSS_A_DICE = (rand() % 10000) / 10000.0; // random between 0 and 1
   if (TOSS_A_DICE < ((mypdf(y1_now2,y2_now2,y3_now2,y4_now2,y5_now2,y6_now2,y7_now2,y8_now2,y9_now2,y10_now2)*(pow(mypdf(y1_now,y2_now,y3_now,y4_now,y5_now,y6_now,y7_now,y8_now,y9_now,y10_now),0.25))) / (mypdf(y1_now,y2_now,y3_now,y4_now,y5_now,y6_now,y7_now,y8_now,y9_now,y10_now)*( pow(mypdf(y1_now2,y2_now2,y3_now2,y4_now2,y5_now2,y6_now2,y7_now2,y8_now2,y9_now2,y10_now2),0.25)))))
       {
	   exchange1 = y1_now2;
	   exchange2 = y2_now2;
	   exchange3 = y3_now2;
...
EX_MCMC=EX_MCMC+expectancy(y1_now,y2_now,y3_now,y4_now,y5_now,y6_now,y7_now,y8_now,y9_now,y10_now);
...
double f8 = (exp(-(pow(((x8-1) /1),2))/2))/1/sqrt(2*pi);
double f9 = (exp(-(pow(((x9-1) /1),2))/2))/1/sqrt(2*pi);
double f10= (exp(-(pow(((x10-1)/1),2))/2))/1/sqrt(2*pi);

pdf = f1*f2*f3*f4*f5*f6*f7*f8*f9*f10;
...
expected = x1*x2*x3*x4*x5*x6*x7*x8*x9*x10;
