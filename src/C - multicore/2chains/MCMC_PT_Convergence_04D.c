...
double mypdf     (double x1,double x2,double x3,double x4);
double expectancy(double x1,double x2,double x3,double x4);
...
if (TOSS_A_DICE < mypdf(y1_c,y2_c,y3_c,y4_c) /mypdf(y1_now,y2_now,y3_now,y4_now) ) 
{
	y1_now = y1_c;
    y2_now = y2_c;
    y3_now = y3_c;
...
TOSS_A_DICE = (rand() % 10000) / 10000.0; // random between 0 and 1
if (TOSS_A_DICE < pow(mypdf(y1_c2,y2_c2,y3_c2,y4_c2),0.25) /pow(mypdf(y1_now2,y2_now2,y3_now2,y4_now2),0.25) )
{
	y1_now2 = y1_c2;
    y2_now2 = y2_c2;
    y3_now2 = y3_c2;
...
// exchange
TOSS_A_DICE = (rand() % 10000) / 10000.0; // random between 0 and 1
   if (TOSS_A_DICE < ((mypdf(y1_now2,y2_now2,y3_now2,y4_now2)*(pow(mypdf(y1_now,y2_now,y3_now,y4_now),0.25))) / (mypdf(y1_now,y2_now,y3_now,y4_now)*( pow(mypdf(y1_now2,y2_now2,y3_now2,y4_now2),0.25)))))
       {
	   exchange1 = y1_now2;
	   exchange2 = y2_now2;
	   exchange3 = y3_now2;
...
EX_MCMC=EX_MCMC+expectancy(y1_now,y2_now,y3_now,y4_now);
